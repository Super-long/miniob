      ...
      if (value_) {
        ...
        for (int i = 1; i < tuple_set.tuples().size(); i++) {
          if (tuple_set.tuples()
                  .at(i)
                  .get(attr_index)
                  .compare(tuple_set.tuples().at(max_index).get(attr_index)) >
              0) {
            max_index = i;
          }
        }
      ...
      if (value_) {
        ...
        for (int i = 1; i < tuple_set.tuples().size(); i++) {
          if (tuple_set.tuples()
                  .at(i)
                  .get(attr_index)
                  .compare(tuple_set.tuples().at(min_index).get(attr_index)) <
              0) {
            min_index = i;
          }
        }
...
RC AggregationNode::init(TupleSchema&& tuple_schema, std::string&& table_name,
                         const char* attr_name, Value* value,
                         int need_table_name, int need_all) {
  tuple_schema_ = tuple_schema;
