...
RC ExecuteStage::do_select(const char* db, Query* sql,
  ...
  if (agg_info.agg_type != AGG_NONE) {
    ...
    if (agg_info.is_constant) value = &agg_info.value;
    ...
    agg_node->init(const_cast<TupleSchema&&>(tuple_set.get_schema()),
                   table_name, attr_name, value, agg_info.need_table_name,
                   agg_info.need_all);
  ...
  for (size_t i = 0; i < selects.relation_num; i++) {
    ...
    LOG_DEBUG("tmp TupleSchema after create_selection_executor : {%s}",
              output.str().c_str());
  ...
  if (tuple_sets.size() > 1) {
    ...
    for (size_t i = 0; i < selects.condition_num; i++) {
      ...
      if (condition.left_is_attr == 1 && condition.right_is_attr == 1 &&
        ...
        size_t index = left_set.schema().index_of_field(
        ...
        index = left_set.schema().index_of_field(
...
static RC schema_add_field(Table* table, const char* field_name,
  ...
  schema.add_if_not_exists(field_meta->type(), table->name(),
                           field_meta->name(), false);

  return RC::SUCCESS;
}

static RC schema_add_field_projection(Table* table, const char* field_name,
                                      TupleSchema& schema) {
  const FieldMeta* field_meta = table->table_meta().field(field_name);
  if (nullptr == field_meta) {
    LOG_WARN("No such field. %s.%s", table->name(), field_name);
...
RC create_selection_executor(Trx* trx, const Selects& selects, const char* db,
  ...
  for (int i = selects.attr_num - 1; i >= 0; i--) {
    ...
    if (nullptr == attr.relation_name ||
      ...
      LOG_DEBUG("attr.relation_name {%s}.  table_name : {%s}",
                attr.relation_name, table_name);
  ...
  for (size_t i = 0; i < selects.condition_num; i++) {
    ...
    if ((condition.left_is_attr == 1 || condition.right_is_attr == 1) &&
      ...
      if (match_table(selects, condition.left_attr.relation_name, table_name)) {
        LOG_DEBUG("lhs schema_add_field : {%s}",
                  condition.left_attr.attribute_name);
        RC rc = schema_add_field_projection(
            table, condition.left_attr.attribute_name, schema);
        if (rc != RC::SUCCESS) {
          return rc;
        }
      }
      if (match_table(selects, condition.right_attr.relation_name,
                      table_name)) {
        LOG_DEBUG("rhs schema_add_field : {%s}",
                  condition.right_attr.attribute_name);
        RC rc = schema_add_field_projection(
            table, condition.right_attr.attribute_name, schema);
        if (rc != RC::SUCCESS) {
          return rc;
        }
      }
