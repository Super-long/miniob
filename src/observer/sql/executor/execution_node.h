...
class SelectExeNode : public ExecutionNode {
 public:
  ...
  // debug
  TupleSchema& return_schema() { return tuple_schema_; }
...
class AggregationNode : public ExecutionNode {
 public:
  ...
  RC init(TupleSchema&& tuple_schema, std::string&& table_name,
          const char* attr_name, Value* value = nullptr,
          int need_table_name = 0, int need_all = 0);
 ...
 private:
  ...
  int need_all_;
  TupleValue* value_; /* [TODO] (adlternative) 可能是 之后是 attr_name + value[]
                         比如 max(id,1,3,4) */
  TupleSchema tuple_schema_;
  AGG_T type_;
