...
int32_t Trx::default_trx_id() { return 0; }
...
void Trx::get_record_trx_id(Table* table, const Record& record, int32_t& trx_id,
                            bool& deleted) {
  const FieldMeta* trx_field = table->table_meta().trx_field();
      ...
      switch (operation.type()) {
        case Operation::Type::INSERT: {
          ...
          if (rc != RC::SUCCESS) {
            // handle rc
            LOG_ERROR(
                "Failed to rollback insert operation. rid=%d.%d, rc=%d:%s",
                rid.page_num, rid.slot_num, rc, strrc(rc));
          }
        } break;
        case Operation::Type::DELETE: {
