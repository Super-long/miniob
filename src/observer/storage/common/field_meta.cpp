...
const static Json::StaticString FIELD_NULLABLE("null_able");
...
FieldMeta::FieldMeta() : attr_type_(AttrType::UNDEFINED), attr_offset_(-1), attr_len_(0), visible_(false), null_able_(false) {
}
...
void FieldMeta::to_json(Json::Value &json_value) const {
  ...
  json_value[FIELD_NULLABLE] = null_able_;
}
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field) {
  ...
  const Json::Value &null_able_value = json_value[FIELD_NULLABLE];
  ...
  if (!null_able_value.isBool()) {
    LOG_ERROR("NullAble field is not a bool value. json value=%s", null_able_value.toStyledString().c_str());
    return RC::GENERIC_ERROR;
  }
  ...
  bool null_able = null_able_value.asBool();
  return field.init(name, type, offset, len, visible, null_able);
}
