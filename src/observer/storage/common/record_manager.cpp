...
int page_fix_size() {
  return sizeof(PageHeader::record_num) + sizeof(PageHeader::record_capacity) +
...
RC RecordPageHandler::init(DiskBufferPool& buffer_pool, int file_id,
  ...
  if ((ret = buffer_pool.get_this_page(file_id, page_num, &page_handle_)) !=
      RC::SUCCESS) {
    LOG_ERROR("Failed to get page handle from disk buffer pool. ret=%d:%s", ret,
              strrc(ret));
    return ret;
  }
...
RC RecordPageHandler::deinit() {
  ...
  if (disk_buffer_pool_ != nullptr) {
    ...
    if (rc != RC::SUCCESS) {
      LOG_ERROR("Failed to unpin page when deinit record page handler. rc=%s",
                strrc(rc));
    }
    disk_buffer_pool_ = nullptr;
  }
...
RC RecordPageHandler::delete_record(const RID* rid) {
  ...
  if (bitmap.get_bit(rid->slot_num)) {
    ...
    ret = disk_buffer_pool_->mark_dirty(&page_handle_);
    if (ret != RC::SUCCESS) {
      LOG_ERROR("failed to mark page dirty in delete record. ret=%d:%s", ret,
                strrc(ret));
      // hard to rollback
    }
...
RC RecordFileHandler::insert_record(const char* data, int record_size,
  ...
  if (current_page_num < 0) {
    if (page_count >= 2) {  // 当前buffer pool 有页面时才尝试加载第一页
      ...
      if ((ret = record_page_handler_.init(*disk_buffer_pool_, file_id_, 1)) !=
          RC::SUCCESS) {
  ...
  if (!page_found) {
    ...
    record_page_handler_.deinit();
    ret = record_page_handler_.init_empty_page(*disk_buffer_pool_, file_id_,
                                               current_page_num, record_size);
    if (ret != RC::SUCCESS) {
...
RC RecordFileScanner::get_next_record(Record* rec) {
  ...
  if ((ret = disk_buffer_pool_->get_page_count(file_id_, &page_count)) !=
      RC::SUCCESS) {
    LOG_ERROR("Failed to get page count while getting next record. file id=%d",
              file_id_);
    return RC::RECORD_EOF;
  }
