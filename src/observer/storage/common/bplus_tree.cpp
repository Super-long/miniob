...
RC BplusTreeHandler::create(const char* file_name, AttrType attr_type,
  ...
  if (rc != SUCCESS) {
    LOG_ERROR("Failed to allocate page. file name=%s, rc=%d:%s", file_name, rc,
              strrc(rc));
    return rc;
  }
  ...
  if (rc != SUCCESS) {
    LOG_ERROR("Failed to get page num. file name=%s, rc=%d:%s", file_name, rc,
              strrc(rc));
    return rc;
  }
...
RC BplusTreeHandler::insert_into_leaf_after_split(PageNum leaf_page,
  ...
  for (i = 0, j = 0; i < leaf->key_num; i++, j++) {
    ...
    memcpy(temp_keys + j * file_header_.key_length,
           leaf->keys + i * file_header_.key_length, file_header_.key_length);
  ...
  for (i = 0; i < split; i++) {
    memcpy(leaf->keys + i * file_header_.key_length,
           temp_keys + i * file_header_.key_length, file_header_.key_length);
...
RC BplusTreeHandler::insert_into_parent(PageNum parent_page, PageNum left_page,
  ...
  if (node->key_num < file_header_.order - 1) {
    ...
    return insert_intern_node_after_split(parent_page, left_page, right_page,
                                          pkey);
  }
}
...
RC BplusTreeHandler::coalesce_node(PageNum leaf_page, PageNum right_page) {
  ...
  if (left->is_leaf == false) {
    memcpy(left->keys + start * file_header_.key_length,
           parent->keys + k * file_header_.key_length, file_header_.key_length);
    start++;
    left->key_num++;
  }
  for (i = start, j = 0; j < right->key_num; i++, j++) {
    memcpy(left->keys + i * file_header_.key_length,
           right->keys + j * file_header_.key_length, file_header_.key_length);
  ...
  if (left->is_leaf)
    ...
    for (i = start; i <= left->key_num; i++) {
      rc = disk_buffer_pool_->get_this_page(file_id_, left->rids[i].page_num,
                                            &tmphandle);
      if (rc != SUCCESS) {
        return rc;
      }
...
RC BplusTreeHandler::redistribute_nodes(PageNum leaf_page, PageNum right_page) {
  ...
  if (left->is_leaf) {
    ...
    if (left->key_num < min_key) {
      memcpy(left->keys + left->key_num * file_header_.key_length, right->keys,
             file_header_.key_length);
    ...
    if (left->key_num < min_key) {
      ...
      rc = disk_buffer_pool_->get_this_page(file_id_, right->rids[0].page_num,
                                            &tmphandle);
      if (rc != SUCCESS) {
        return rc;
      }
...
RC BplusTreeHandler::delete_entry_internal(PageNum page_num, const char* pkey) {
  ...
  if (node->parent == -1) {
    if (node->key_num == 0 && node->is_leaf == false) {
      rc = disk_buffer_pool_->get_this_page(file_id_, node->rids[0].page_num,
                                            &tmphandle);
      if (rc != SUCCESS) {
        return rc;
      }
